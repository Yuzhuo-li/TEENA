...
def draw_volcano(df_enh_all, prefix):
    ...
    for category in four_category:
        ...
        axs[index].set_ylabel('-log10(P)', fontweight='bold')
        ...
        axs[index].axhline(y=1.30103, color='dimgrey',
                           linestyle='dashed', linewidth=1)
        axs[index].set_xticks([-8, -6, -4, -2, -1, 0, 1, 2, 4, 6, 8])
        axs[index].set_xlim([-8, 8])
...
def draw_bar(df_enh_all, prefix):
    ...
    vmin = min(min(row) for row in f_list if row)
    vmax = max(max(row) for row in f_list if row)
